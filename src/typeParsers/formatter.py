...
def format_bonuses(bonuses: str) -> list:
    ...
    current_bonus = dict()
    first_line = bonus_lines.pop(0)
    current_field = "lead-in"
    current_bonus[current_field] = remove_bad_stuff(first_line)
    while len(bonus_list) > 0:
        next_line = bonus_list.pop(0)
        if next_line[:3] == "[10":
            current_field = "question-" + str((len(current_bonus) + 1) / 2)
            current_bonus[current_field] = remove_bad_stuff(next_line)
        elif next_line[:7].lower() == "answer:":
            current_field = "answer-" + str(len(current_bonus) / 2)
            current_bonus[current_field] = remove_bad_stuff(next_line)
        elif starts_with_number(next_line):
